...
if __name__ == '__main__':
    ...
    train, test = dp.vn_drop_id(train, test)
    ...
    train["Price"] = np.log1p(train["Price"])  # np.log_e(1+x), e= 2.71828182846

    # Concat train & test data
    all_data, y_train, n_train, n_test = dp.vn_concat_data(train, test)

    # Imputing missing values
    all_data = dp.vn_imputing_missing_values(all_data)  # optional ???
    ...
    all_data = dp.vn_transform_numerical_to_categorical_values(all_data)  # customize manually
